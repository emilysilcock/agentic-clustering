...
def generate_topics(
    ...
    # Vendored addition: per-doc checkpoint. If both df_full and out_file
    # are provided, writes partial topics + responses after every iteration
    # so a mid-loop kill doesn't lose subscription compute. Falls back to
    # upstream behaviour (no checkpoint) when either is None.
    df_full=None,
    out_file=None,
    topic_file=None,
):
    ...
    def _checkpoint() -> None:
        # Vendored addition: write partial state to disk so an
        # interrupted run can be inspected (and the SubscriptionCompute
        # already paid for is recoverable).
        if df_full is None or out_file is None or topic_file is None:
            return
        if not responses:
            return
        try:
            topics_root.to_file(topic_file)
            partial = df_full.iloc[: len(responses)].copy()
            partial["responses"] = responses
            partial.to_json(out_file, lines=True, orient="records")
...
def generate_topic_lvl1(
    ...
    # Generate topics
    # Vendored addition: pass df_full + out_file + topic_file so generate_topics
    # can write partial state after every iteration --- protects against
    # losing Opus subscription compute on a mid-loop kill.
    ...
    try:
        ...
        traceback.print_exc()
        with open(f"data/output/generation_1_backup_{model}.txt", "w", encoding="utf-8") as f:
