...
description: >
  ...
  score. Use to validate whether clusters hold up against unseen data.
...
Your workflow:
1. Read `.claude/clustering/state.json` to get current cluster definitions.
   ...
   definitions and the instructions. If a text could fit a cluster technically
   but doesn't align with the spirit of the instructions, note that.
...
4. Write your full audit to
   `.claude/clustering/audits/audit_{YYYYMMDD_HHMMSS}_{uuid4_short}.json`
...
```json
{
  ...
  "summary": {
    ...
    "mean_confidence": 3.9,
    "per_cluster": {
      "c1": {"count": 15, "mean_confidence": 4.3},
      "c3": {"count": 6, "mean_confidence": 2.8}
    },
    "weak_clusters": ["c3"],
