...
`/cluster-label` writes labels.json as a list of `{id, cluster, text}`
objects (one per hand-labelled text). `/cluster-tune` needs to run the
same hand-labelled texts through classify.py, which expects an input
of `{id, text}` objects with no cluster column. This script does the
extraction so each tuning run doesn't reinvent it.

Accepts either labels.json shape:
  - List form: ``[{"id": ..., "cluster": ..., "text": ...}, ...]``
  - Dict form: ``{"id1": "cluster_id", "id2": "none", ...}`` — only useful
...
def _load_labels(path: Path) -> list[dict]:
    ...
    if isinstance(data, dict):
        ...
        return [{"id": str(k), "cluster": str(v)} for k, v in data.items()]
    print(
...
def _load_corpus_lookup(path: Path) -> dict[str, str]:
    ...
    init.py) or a similar [{id, text}, ...] list."""
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
...
def main() -> int:
    ...
    p.add_argument("--labels", required=True, help="Path to labels.json (from /cluster-label)")
    p.add_argument(
        "--output",
        required=True,
    ...
    p.add_argument(
        ...
        help=(
            ...
            "no `text` field). Usually $CLUSTERING_WORKSPACE/corpus.json."
        ),
    )
    args = p.parse_args()
