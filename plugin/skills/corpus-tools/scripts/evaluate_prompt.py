...
Reads a predictions CSV (output of classify.py) and a labels JSON, joins on ID,
and reports overall accuracy, per-cluster precision/recall/F1, and the list of
disagreements. Writes a JSON report to stdout (or --output path).

Labels JSON is one of:
  - {"id1": "c3", "id2": "none", ...}
  - [{"id": "id1", "cluster": "c3"}, {"id": "id2", "cluster": "none"}, ...]
"""
...
def load_labels(path: Path) -> dict[str, str]:
    ...
    if isinstance(data, list):
        ...
        for item in data:
            ...
            cluster = item.get("cluster", item.get("label"))
...
def load_predictions(path: Path, id_col: str, cluster_col: str) -> dict[str, str]:
    out = {}
